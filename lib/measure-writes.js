...
function nop(err) {
  if (!err) { console.log('success measure write'); return; }
  console.warn(err);
};
...
try {
  for (var i = 0; i < argv.i; i++) {
    fs.writeFile("/sys/class/gpio/gpio16/value", argv.value, nop);
  }
  report(name, b);
} catch (e) {
  console.log("%s failed", name);
...
try {
  for (var i = 0; i < argv.i; i++) {
    fs.writeFileSync("/sys/class/gpio/gpio16/value", argv.value, nop);
  }
  report(name, b);
} catch (e) {
  console.log("%s failed", name);
